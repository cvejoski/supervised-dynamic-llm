"""
    Implementation of RetinaNet object detector.
...
class FocalLoss(nn.Module):
    def __init__(self, alpha: float = 0.25, gamma: float = 2.0, device: torch.device = "cpu"):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
        self.device = device
        self.reg_loss = nn.SmoothL1Loss(reduction="mean", beta=1.0 / 9.0)
    ...
    def forward(self, classifications, regressions, anchors, annotations):
        ...
        for j in range(batch_size):
            ...
            if bbox_annotation.shape[0] == 0:
                ...
                alpha_factor = torch.ones(classification.shape, device=self.device) * self.alpha
                ...
                regression_losses.append(torch.tensor(0, device=self.device).float())
            ...
            if positive_indices.sum() > 0:
                ...
                regression_loss = self.reg_loss(input=regression[positive_indices, :], target=targets)
...
class RetinaNet(AModel):
    def __init__(self, **kwargs):
        ...
        self.__init_components(num_classes, depth, pretrained)

    def __init_components(self, num_classes: int, depth: str, pretrained: bool = False):
        if depth in retinanet_settings.keys():
            block = retinanet_settings[depth]["block"]
            layers = retinanet_settings[depth]["layers"]
        else:
            raise NotImplementedError(f"RetinaNet depth '{depth}' not implemented. Choose from {list(retinanet_settings.keys())}.")
        ...
        if pretrained:
            self.load_state_dict(model_zoo.load_url(model_urls[f"resnet{depth}"], model_dir="."), strict=False)
    ...
    def forward(self, inputs):
        ...
        if self.training:
            img_batch, targets = inputs
            annotations = [torch.hstack((t["boxes"], t["labels"][:, None])).to(self.device) for t in targets]
            img_batch.to(self.device)
        else:
            if isinstance(inputs, list):
                img_batch = torch.stack(inputs)
            elif isinstance(inputs, torch.Tensor):
                img_batch = inputs
            else:
                raise ValueError(f"Wrong input type: {type(inputs)}")
        ...
        regression = torch.cat([self.regressionModel(feature) for feature in features], dim=1).to(self.device)

        classification = torch.cat([self.classificationModel(feature) for feature in features], dim=1).to(self.device)

        anchors = self.anchors(img_batch).to(self.device)

        if self.training:
            return FocalLoss(alpha=self.alpha, gamma=self.gamma, device=self.device)(classification, regression, anchors, annotations)
        else:
            ...
            finalImageIndexes = torch.Tensor([]).to(self.device)
            ...
            for i in range(classification.shape[2]):
                ...
                idx = torch.where(scores_over_thresh)[0]

                anchorBoxes = transformed_anchors
                anchorBoxes = anchorBoxes[scores_over_thresh]
                anchors_nms_idx = nms(anchorBoxes, scores, 0.5)

                finalScores = torch.cat((finalScores, scores[anchors_nms_idx]))
                finalAnchorBoxesIndexesValue = (torch.tensor([i] * anchors_nms_idx.shape[0])).to(self.device)

                finalAnchorBoxesIndexes = torch.cat((finalAnchorBoxesIndexes, finalAnchorBoxesIndexesValue))
                finalAnchorBoxesCoordinates = torch.cat((finalAnchorBoxesCoordinates, anchorBoxes[anchors_nms_idx]))

                finalImageIndexes = torch.cat((finalImageIndexes, idx[anchors_nms_idx]))

            resultDict = [
                {
                    "scores": finalScores[finalImageIndexes == i],
                    "labels": finalAnchorBoxesIndexes[finalImageIndexes == i],
                    "boxes": finalAnchorBoxesCoordinates[finalImageIndexes == i],
                }
    ...
    def loss(self, x: torch.Tensor, y_target: torch.Tensor) -> dict:
        # Toggle train mode
        self.training = True
        return self.forward(inputs=(x, y_target))
    ...
    def train_step(self, minibatch: Tuple[torch.Tensor], optimizer: dict, step: int, scheduler: Any = None) -> dict:
        ...
        # Train loss
        loss_dict = self.loss(
            x=torch.stack([image.to(self.device, non_blocking=True) for image in images]),
            y_target=[{k: v.to(self.device) for k, v in t.items()} for t in targets],
        )
    ...
    def validate_step(self, minibatch: Tuple[torch.Tensor], iou_thresh: float) -> dict:
        ...
        images, targets = minibatch
        x = list(image.to(self.device, non_blocking=True) for image in images)
    ...
    @staticmethod
    def target_transform(annotations: List[dict], name2class: dict) -> dict:
